...
def build_key(d):
    ...
    age = d['Age Cohort']
    mt = d['Measure Type']
    return '{}_{}_{}_{}_{}'.format(county, year, gender, age, mt)
...
sorted_complete = multikeysort(
    complete,
    ['Year', 'County', 'Gender', 'Race/Ethnicity', 'Age Cohort', 'Measure Type', 'Variable'])

with open('data/population-by-age-by-county.csv', 'w') as f:
    writer = csv.DictWriter(
        f,
        ['Year', 'FIPS', 'County', 'Gender', 'Race/Ethnicity', 'Age Cohort', 'Measure Type', 'Variable', 'Value'])
    writer.writeheader()
