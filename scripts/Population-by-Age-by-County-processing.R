...
##################################################################
#
# Processing Script for Population by Age by County
...
dataset <- dataset %>% 
  select(County, FIPS, Year, Gender, `Race/Ethnicity`, `Age Cohort`, `Measure Type`, Variable, Value) %>% 
  arrange(County, Year, Gender, `Race/Ethnicity`, `Age Cohort`, `Measure Type`, desc(Variable))

write.table(
    dataset,
    file.path("data", "population-by-age-county-2018.csv"),
    sep = ",",
    row.names = F,
    na = "-9999"
)
