...
def build_key(d):
    ...
    mt = d['Measure Type']
    variable = d['Variable']
    return '{}_{}_{}_{}_{}_{}_{}'.format(county, year, race, gender, age, mt, variable)

def build_county_total_key(county, year, variable):
    return '{}_{}_All_Total_Total_Number_{}'.format(county, year, variable)
...
with open('raw/population-by-age-by-county.csv') as f:
...
for row in population:
    if row['Variable'] == 'Population':
        ...
        total_lookup_key = build_county_total_key(row['County'], row['Year'], 'Population')
        total_moe_lookup_key = build_county_total_key(row['County'], row['Year'], 'Margins of Error')
...
with open('raw/population-by-age-recalculated.csv', 'w') as f:
    writer = csv.DictWriter(f, fieldnames=['County', 'FIPS', 'Year', 'Gender', 'Race/Ethnicity', 'Age Cohort', 'Measure Type', 'Variable', 'Value'])
    writer.writeheader()
